...
// Pagination window - hiển thị tối đa 7 trang
...
const CategoryPage = () => {
    ...
    const selectedProductType = searchParams.get('productType') || null;
    ...
    const debouncedSetSearchParams = useCallback(
        ...
        [selectedProductType, sortBy, urlMinPrice, urlMaxPrice, selectedColor, selectedMaterial, urlMinWidth, urlMaxWidth, urlMinHeight, urlMaxHeight, urlMinDepth, urlMaxDepth, setSearchParams]
    );
    ...
    // Debounced dimensions update (300ms)
    const debouncedApplyDimensions = useCallback(
        ...
        [selectedProductType, sortBy, urlMinPrice, urlMaxPrice, selectedColor, selectedMaterial, urlSearch, urlMinWidth, urlMaxWidth, urlMinHeight, urlMaxHeight, urlMinDepth, urlMaxDepth, setSearchParams]
    );
    ...
    // Fetch category, product types, colors, materials khi id thay đổi
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch Colors for this category
                const colorsResponse = await fetch(`/api/products/colors?categoryId=${id}`);
                ...
                const materialsResponse = await fetch(`/api/products/materials?categoryId=${id}`);
    ...
    const fetchProducts = useCallback(async (
        ...
        try {
            ...
            const params = new URLSearchParams({ categoryId: id, page, pageSize, sortBy: sort });
    ...
    }, [id, selectedProductType, sortBy, urlMinPrice, urlMaxPrice, selectedColor, selectedMaterial, urlSearch, urlMinWidth, urlMaxWidth, urlMinHeight, urlMaxHeight, urlMinDepth, urlMaxDepth]);

    // Fetch products on filter changes
    useEffect(() => {
        fetchProducts(currentPage, selectedProductType, sortBy, urlMinPrice, urlMaxPrice, selectedColor, selectedMaterial, urlSearch, urlMinWidth, urlMaxWidth, urlMinHeight, urlMaxHeight, urlMinDepth, urlMaxDepth);
    }, [currentPage, id, selectedProductType, sortBy, urlMinPrice, urlMaxPrice, selectedColor, selectedMaterial, urlSearch, urlMinWidth, urlMaxWidth, urlMinHeight, urlMaxHeight, urlMinDepth, urlMaxDepth, fetchProducts]);
    ...
    const handlePageChange = (page) => {
        if (page >= 1 && page <= totalPages) {
            fetchProducts(page, selectedProductType, sortBy, urlMinPrice, urlMaxPrice, selectedColor, selectedMaterial, urlSearch, urlMinWidth, urlMaxWidth, urlMinHeight, urlMaxHeight, urlMinDepth, urlMaxDepth);
                                ...
                                <div className="filter-popover style-popover">
                                    ...
                                    <div className="popover-styles">
                                        {[
                                            ...
                                            { name: 'Hiện đại', label: '🏙️ Hiện đại (Modern)' },
                                            { name: 'Cổ điển', label: '🏛️ Tân cổ điển' },
                ...
                <div className={`all-filters-drawer ${isDrawerOpen ? 'active' : ''}`} onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="drawer-list-options">
                                {rootCategories.map(cat => (
                                    <Link
                                        ...
                                        className={`drawer-option-btn ${cat.categoryId === parseInt(id) ? 'active' : ''}`}
                    ...
                    <div className="drawer-footer">
                        ...
                        <button className="btn-submit" onClick={() => setIsDrawerOpen(false)}>
                            Hiển thị {totalProducts} kết quả
                        </button>
                    </div>
                </div>
            </div>
