...
const AdminDashboard = () => {
  ...
  const [formData, setFormData] = useState({
    name: '', description: '', basePrice: '', discountPrice: '', costPrice: '',
  ...
  const fetchCategories = async () => {
    try {
      const r = await fetch(buildUrl('/api/categories'));
  ...
  const uploadImage = async (productId) => {
    ...
    try {
      const r = await fetch(buildUrl(`/api/products/${productId}/upload-image`), { method: 'POST', headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` }, body: fd });
  ...
  const handleDelete = async (productId) => {
    ...
    const r = await fetch(buildUrl(`/api/products/${productId}`), { method: 'DELETE', headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` } });
    ...
    <AdminLayout>
      ...
      <Modal show={showModal} onClose={closeModal} title={modalMode === 'add' ? 'Thêm sản phẩm mới' : 'Chỉnh sửa sản phẩm'} size="lg">
        <FormBody onSubmit={handleSubmit}>
          ...
          <FormGroup label="Giá vốn (VNĐ)">
            <FormInput type="number" name="costPrice" value={formData.costPrice} onChange={handleChange} min="0" />
          </FormGroup>
