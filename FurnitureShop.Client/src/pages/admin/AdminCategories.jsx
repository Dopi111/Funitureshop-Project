...
const AdminCategories = () => {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const r = await fetch(buildUrl('/api/categories/all'));
  ...
  const uploadImage = async (categoryId) => {
    ...
    try {
      const r = await fetch(buildUrl(`/api/categories/${categoryId}/upload-image`), { method:'POST', headers:{'Authorization':`Bearer ${localStorage.getItem('authToken')}`}, body:fd });
  ...
  const handleDeleteImage = async (categoryId) => {
    ...
    const r = await fetch(buildUrl(`/api/categories/${categoryId}/image`), { method:'DELETE', headers:{'Authorization':`Bearer ${localStorage.getItem('authToken')}`} });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const r = await fetch(buildUrl(url), { method:modalMode==='add'?'POST':'PUT', headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`}, body:JSON.stringify(modalMode==='edit'?{...payload,categoryId:selectedCategory.categoryId}:payload) });
  ...
  const handleDelete = async (categoryId) => {
    ...
    const r = await fetch(buildUrl(`/api/categories/${categoryId}`), { method:'DELETE', headers:{'Authorization':`Bearer ${localStorage.getItem('authToken')}`} });
  ...
  const fetchProductTypes = async (cat) => {
    try { setPtLoading(true); setViewingCategory(cat);
      const r = await fetch(buildUrl(`/api/products/product-types/by-category/${cat.categoryId}`));
      if (r.ok) setProductTypes(await r.json() || []);
    } catch { setProductTypes([]); } finally { setPtLoading(false); }
  };
