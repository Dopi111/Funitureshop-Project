...
const AdminInventory = () => {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          const mapped = list.map(p => ({
            ...
            reserved: p.reservedQuantity ?? p.allocatedStock ?? null,
  ...
  const filtered = items.filter(i => (!filter || i.status === filter) && (!search || i.name.toLowerCase().includes(search.toLowerCase()) || i.sku.toLowerCase().includes(search.toLowerCase())));

  const totalValue = items.some(i => i.cost !== null && i.cost !== undefined)
    ? items.reduce((s, i) => s + (i.stock * (i.cost ?? 0)), 0)
    : null;
  ...
  const saveEdit = async (id) => {
    ...
    const res = await apiService.request(`/Inventory/products/${id}/adjust`, {
      method: 'POST',
      body: JSON.stringify({ quantity: n, reason: 'Kiểm kê nhanh từ trang quản trị' }),
    });
    if (!res.success) {
      toast.error(res.message || 'Không thể cập nhật tồn kho');
      return;
    }
    setItems(prev => prev.map(i => i.id === id ? { ...i, stock: n, status: n === 0 ? 'cancelled' : n <= 5 ? 'warning' : 'completed' } : i));
    setEditing(null);
  ...
  return (
    <AdminLayout>
      <PageHeader title="Quản Trị Kho Bãi" subtitle="Hệ thống giám sát tồn kho thời gian thực & Kiểm soát định giá COGS" breadcrumb={['Admin', 'Kho & Vận chuyển', 'Tồn kho hiện tại']}>
        <Btn variant="primary" onClick={() => navigate('/admin/stockin')}>+ Tạo Phiếu Nhập Hàng</Btn>
      </PageHeader>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        <StatCard icon="📦" label="Tổng Sản Phẩm Tồn" value={`${fmtNum(totalItems)} SP`} sub="Khả dụng lưu kho" />
        <StatCard icon="💰" label="Tổng Giá Trị Vốn" value={totalValue === null ? 'N/A' : fmt(totalValue)} sub="Chỉ tính item có cost thật" />
      ...
      <div className="bg-white rounded-2xl border border-zinc-200/80 shadow-xs overflow-hidden">
        <div className="p-4 border-b border-zinc-100 bg-zinc-50/50 flex flex-wrap items-center justify-between gap-3">
          <div className="relative flex-1 min-w-[240px] max-w-md">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Tìm kiếm theo mã SKU, tên sản phẩm nội thất..." className="w-full bg-white pl-10 pr-4 py-2 rounded-xl border border-zinc-200 text-xs outline-none focus:border-black transition-all font-['Outfit']" />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[840px]">
            ...
            <tbody className="divide-y divide-zinc-100">
              {filtered.map(item => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-zinc-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-xs font-mono font-bold text-center">
                      {isEdit ? (
                        <div className="inline-flex items-center gap-1">
                          <input type="number" value={editVal} onChange={e => setEditVal(e.target.value)} min={0} className="w-16 px-2 py-1 bg-white border border-black rounded-lg text-xs font-mono outline-none text-center" autoFocus onKeyDown={e => { if (e.key === 'Enter') saveEdit(item.id); if (e.key === 'Escape') setEditing(null); }} />
                    ...
                    <td className="px-4 py-3.5 text-xs font-mono text-zinc-400 text-center">{item.reserved === null || item.reserved === undefined ? 'N/A' : item.reserved}</td>
                    <td className="px-4 py-3.5 text-xs font-mono font-bold text-zinc-900 text-right">{fmt(item.price)}</td>
                    <td className="px-4 py-3.5 text-xs font-mono text-zinc-500 text-right">{item.cost === null || item.cost === undefined ? 'N/A' : fmt(item.cost)}</td>
                    <td className="px-4 py-3.5 text-center"><Badge variant={item.status}>{item.status === 'completed' ? 'CÒN HÀNG' : item.status === 'warning' ? 'SẮP HẾT' : 'HẾT HÀNG'}</Badge></td>
                    <td className="px-4 py-3.5 text-right">
                      <button onClick={() => { setEditing(item.id); setEditVal(String(item.stock)); }} className="px-3 py-1 bg-white border border-zinc-200 rounded-lg text-[11px] font-medium text-zinc-700 hover:border-black hover:bg-black hover:text-white transition-all cursor-pointer shadow-2xs">Kiểm kê</button>
                    </td>
                  </tr>
                );
              })}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={9} className="py-12 text-center text-xs text-zinc-400 font-mono">{loading ? 'ĐANG TẢI DỮ LIỆU TỒN KHO TỪ HỆ THỐNG...' : 'KHÔNG CÓ DỮ LIỆU SẢN PHẨM HOẶC KHÔNG KHỚP BỘ LỌC'}</td>
                </tr>
              )}
            </tbody>
          </table>
