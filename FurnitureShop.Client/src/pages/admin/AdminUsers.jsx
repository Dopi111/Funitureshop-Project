...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const r = await fetch(buildUrl(`/api/auth/users?${params}`), { headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const r = await fetch(buildUrl(url), { method: modalMode === 'add' ? 'POST' : 'PUT', headers: { 'Content-Type':'application/json','Authorization':`Bearer ${token}` }, body: JSON.stringify(modalMode === 'edit' ? {...payload,userId:selectedUser.userId} : payload) });
  ...
  const handleToggleStatus = async (userId, current) => {
    ...
    const r = await fetch(buildUrl(`/api/auth/users/${userId}/toggle-status`), { method: 'PATCH', headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` } });
  ...
  const handleDelete = async (userId) => {
    ...
    const r = await fetch(buildUrl(`/api/auth/users/${userId}`), { method: 'DELETE', headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` } });
  ...
  const handleResetPassword = async (userId) => {
    ...
    const r = await fetch(buildUrl(`/api/auth/users/${userId}/reset-password`), { method: 'POST', headers: { 'Content-Type':'application/json','Authorization':`Bearer ${localStorage.getItem('authToken')}` }, body: JSON.stringify({ newPassword: pw }) });
